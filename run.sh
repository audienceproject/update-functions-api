...
for type_file in `echo $type_files | tr "," "\n"`; do
    ...
    curl -H "Auth: Bearer $api_oauth_token" -H "Content-Type: application/json" -X PUT -d @$type_file $api_endpoint/types/$type_name
done

# Update Function API with function
function_name=$(basename "$function_file" .json)
curl -H "Auth: Bearer $api_oauth_token" -H "Content-Type: application/json" -X PUT -d @$function_file $api_endpoint/functions/$function_name
